      ...
      <header className="glass-header py-3 sm:py-4 px-4 sm:px-8 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto flex justify-between items-center w-full">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[var(--color-accent)] to-[var(--color-secondary)] text-white flex items-center justify-center font-bold shadow-[0_0_16px_var(--color-accent-glow)] border border-white/40">
              S
            </div>
            <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
              <div className="flex items-center gap-4">
                <button 
                  ...
                  className="btn btn-secondary text-sm px-4 py-2.5 rounded-xl self-start sm:self-auto"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                  ...
                  {selectedProcess.description && (
                    <p className="text-sm text-[var(--color-text-secondary)] mt-1">{selectedProcess.description}</p>
                  )}
                </div>
              </div>
            </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-medium text-white text-base">{l.title}</h3>
                        <span className="text-xs px-2.5 py-0.5 rounded-full bg-[rgba(255,255,255,0.1)] border border-white/10 text-[var(--color-text-secondary)]">
                          {l.lessonType}
                        </span>
                      </div>
                      {l.description && (
          ...
          <>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
              <div className="text-center sm:text-left">
                <h2 className="text-2xl sm:text-3xl font-bold text-white font-heading">My Courses</h2>
                <p className="text-sm text-[var(--color-text-secondary)] mt-1">Select a course to continue learning</p>
              </div>
            </div>

            {assignments.length === 0 ? (
              <div className="text-center py-16 px-4 glass-card max-w-md mx-auto animate-fade-in">
                <div className="text-6xl mb-4">📚</div>
                <h3 className="text-xl font-bold text-white font-heading mb-2">No courses assigned yet</h3>
                <p className="text-sm text-[var(--color-text-secondary)]">Wait for your teacher to assign a course to you.</p>
              </div>
            ) : (
              <div className="grid gap-5 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {assignments.map((a, idx) => (
                  <div key={a.id} className="quiz-card animate-fade-in flex flex-col justify-between" style={{ animationDelay: `${idx * 0.1}s` }}>
                    <div>
